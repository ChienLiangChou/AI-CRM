...
def _validate_schema(schema_cls: Any, payload: Any):
...
def _write_transition_audit_log(
    ...
    *,
    ...
    details = {
        ...
        "session_reference": attempt.session_reference if attempt is not None else None,
...
def load_persisted_history(
    ...
    *,
    ...
    attempts = [
        attempt
        for attempt in (_parse_attempt_from_run(run) for run in runs)
        if attempt is not None
    ]
...
def submit_otp_for_attempt_persisted(
    ...
    *,
    ...
    if (
        status.state == "failed"
        and attempt.failure_reason == "otp_timeout"
        and request.attempt_reference == attempt.attempt_reference
        and request.session_reference == attempt.session_reference
    ):
        return agent_schemas.MlsAuthSubmitOtpResponse(
            status=status,
            attempt=attempt,
...
def expire_session_persisted(
    ...
    *,
    ...
    latest_run, latest_attempt = _get_latest_attempt_run(db, task=task)
    next_status = expire_session(status=status, now=timestamp)
    _persist_status_snapshot(
        db,
        task=task,
        status=next_status,
    ...
    _write_transition_audit_log(
        ...
        task=task,
        run=latest_run,
        action="mls_auth_session_expired",
        provider=provider,
        from_state=status.state,
        to_state=next_status.state,
        attempt=latest_attempt,
        failure_reason=next_status.failure_reason,
        extra={"current_session_reference": status.session_reference},
    )
    return next_status
