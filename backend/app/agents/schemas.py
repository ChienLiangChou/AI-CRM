...
DailyMarketScanDraftType = Literal[
    "client_match_update",
    "competitor_watch_update",
]
DailyMarketScanDraftStatus = Literal[
    "pending_generation",
    "approval_created",
    "skipped",
    "failed",
]
DailyMarketScanNoteStatus = Literal["pending", "attached", "skipped", "failed"]
...
class DailyMarketScanNoteAttachmentPlan(BaseModel):
    status: DailyMarketScanNoteStatus = "pending"
    target_mode: Literal["contact_internal_note"] = "contact_internal_note"
...
class DailyMarketScanDraftProposal(BaseModel):
    draft_type: DailyMarketScanDraftType
    review_mode: Literal["manual_only"] = "manual_only"
    status: DailyMarketScanDraftStatus = "pending_generation"
...
class DailyMarketScanClientMatchScan(BaseModel):
    ...
    findings: list[DailyMarketScanFinding] = []
    note_attachment: Optional[DailyMarketScanNoteAttachmentPlan] = None
    draft_proposal: Optional[DailyMarketScanDraftProposal] = None
    fallback_used: bool = False
    failure_metadata: list[DailyMarketScanFailureMetadata] = []
