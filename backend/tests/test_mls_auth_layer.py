...
class MlsAuthLayerTests(unittest.TestCase):
    ...
    def test_start_auth_attempt_creates_auth_in_progress_with_non_secret_references(self):
        ...
        self.assertEqual(
            response.status.active_attempt_reference,
            response.attempt.attempt_reference,
        )
        self.assertEqual(
            response.status.session_reference,
            response.attempt.session_reference,
        )
        self.assertTrue(
            response.status.session_reference.startswith(
                "mls_auth_session_stratus_authenticated_"
            )
        )
        self.assertTrue(
            response.attempt.attempt_reference.startswith(
                "mls_auth_attempt_stratus_authenticated_"
            )
        )
    ...
    def test_start_auth_attempt_reuses_existing_active_attempt(self):
        ...
        self.assertEqual(
            second.status.session_reference,
            first.status.session_reference,
        )
    ...
    def test_start_auth_attempt_reuses_existing_awaiting_otp_attempt(self):
        ...
        self.assertEqual(
            reused.attempt.attempt_reference,
            awaiting_attempt.attempt_reference,
        )
...
class MlsAuthPersistenceTests(unittest.TestCase):
    ...
    def test_persisted_history_returns_newest_attempt_first(self):
        ...
        self.assertEqual(
            history.current_status.active_attempt_reference,
            second.attempt.attempt_reference,
        )

    def test_session_expired_is_persisted_without_rewriting_attempt_history(self):
        now = utcnow_naive()
        started = mls_auth.start_auth_attempt_persisted(
            self.db,
            request=agent_schemas.MlsAuthStartRequest(),
        ...
        status, _attempt = mls_auth.mark_auth_available_persisted(
            self.db,
            provider="stratus_authenticated",
            attempt_reference=started.attempt.attempt_reference,
            now=now + timedelta(minutes=1),
        ...
        expired_status = mls_auth.expire_session_persisted(
            self.db,
            provider="stratus_authenticated",
        ...
        self.assertEqual(status.state, "available")
        self.assertEqual(expired_status.state, "expired")
        self.assertEqual(expired_status.failure_reason, "session_expired")
        self.assertEqual(task.status, "failed")
        self.assertEqual(json.loads(task.payload)["current_status"]["state"], "expired")
        self.assertEqual(run.status, "completed")
        self.assertEqual(run_result["attempt"]["state"], "available")
        self.assertEqual(audits[-1].action, "mls_auth_session_expired")

    def test_route_surface_returns_safe_empty_status_history_and_audit(self):
        status = agent_router.get_mls_auth_status(db=self.db)
        history = agent_router.get_mls_auth_history(db=self.db)
        audit_logs = agent_router.list_mls_auth_audit_logs(db=self.db)

        self.assertEqual(status.state, "unauthenticated")
        self.assertFalse(status.available)
        self.assertEqual(history.current_status.state, "unauthenticated")
        self.assertEqual(history.attempts, [])
        self.assertEqual(audit_logs, [])

    def test_route_start_and_status_reuse_active_attempt(self):
        first = agent_router.start_mls_auth_attempt(
            agent_schemas.MlsAuthStartRequest(),
            db=self.db,
        )
        second = agent_router.start_mls_auth_attempt(
            agent_schemas.MlsAuthStartRequest(),
            db=self.db,
        )
        status = agent_router.get_mls_auth_status(db=self.db)
        ...
        self.assertFalse(first.reused_existing_attempt)
        self.assertTrue(second.reused_existing_attempt)
        self.assertEqual(
            second.attempt.attempt_reference,
            first.attempt.attempt_reference,
        )
        self.assertEqual(status.state, "auth_in_progress")
        self.assertEqual(len(history.attempts), 1)

    def test_route_submit_otp_fails_safely_on_reference_mismatch(self):
        started = agent_router.start_mls_auth_attempt(
            agent_schemas.MlsAuthStartRequest(),
            db=self.db,
        )
        _status, awaiting_attempt = mls_auth.mark_otp_required_persisted(
            self.db,
            provider="stratus_authenticated",
            attempt_reference=started.attempt.attempt_reference,
            now=utcnow_naive(),
        ...
        with self.assertRaises(HTTPException) as error:
            agent_router.submit_mls_auth_otp(
                agent_schemas.MlsAuthSubmitOtpRequest(
                    provider="stratus_authenticated",
                    attempt_reference=awaiting_attempt.attempt_reference,
                    session_reference="bad_session_ref",
                    otp_code="123456",
        ...
        self.assertEqual(error.exception.detail, "session_reference_mismatch")

    def test_route_submit_otp_fails_soft_on_timeout(self):
        now = utcnow_naive()
        started = agent_router.start_mls_auth_attempt(
            agent_schemas.MlsAuthStartRequest(),
            db=self.db,
        ...
        with patch(
            "app.agents.mls_auth._utcnow_naive",
            return_value=(awaiting_status.otp_timeout_at or now) + timedelta(seconds=1),
        ):
            response = agent_router.submit_mls_auth_otp(
                agent_schemas.MlsAuthSubmitOtpRequest(
                    provider="stratus_authenticated",
                    attempt_reference=awaiting_attempt.attempt_reference,
                    session_reference=awaiting_attempt.session_reference,
        ...
        self.assertFalse(response.otp_accepted)
        self.assertEqual(response.status.state, "failed")
        self.assertEqual(response.status.failure_reason, "otp_timeout")
        self.assertEqual(status.state, "failed")
        self.assertEqual(status.failure_reason, "otp_timeout")

    def test_route_history_and_audit_logs_are_ordered_and_scoped(self):
        now = utcnow_naive()
        first = agent_router.start_mls_auth_attempt(
            agent_schemas.MlsAuthStartRequest(),
            db=self.db,
        ...
        self.assertEqual(audit_logs[0].action, "mls_auth_attempt_started")
        self.assertIn(audit_logs[-1].action, {"mls_auth_attempt_started", "mls_auth_failed"})
